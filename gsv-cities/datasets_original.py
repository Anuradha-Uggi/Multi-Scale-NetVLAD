...
def input_transform(resize=(320, 240)):
...
class PlaceDataset(data.Dataset):
    def __init__(self, query_file_path, index_file_path, dataset_root_dir, ground_truth_path, config, ttype, lent, seed, posDistThr=None):
        super().__init__()
        ...
        self.images_orig = [os.path.join(dataset_root_dir, image) for image in self.images]
        
        np.random.seed(seed)
        
        if self.ttype=='qry':
           print(f'===>{self.ttype} activated')
           self.images = self.images_orig
        elif self.ttype=='db':   
           print(f'===>{self.ttype} activated')
           self.images_orig_np = np.array(self.images_orig)
           sampler = np.random.choice(len(self.images_orig), lent, replace=False)
           print(sampler)
           self.images = self.images_orig_np[sampler]
        else:
           print(f'===>{self.ttype} deactivated')
           self.images = self.images_orig    
